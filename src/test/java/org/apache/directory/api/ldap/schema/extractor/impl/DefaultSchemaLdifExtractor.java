...
//Copyright Apache Software Foundation (ASF) - Original license ASL2.
...
public class DefaultSchemaLdifExtractor implements SchemaLdifExtractor {
     ...
     * java.util.regex.Pattern is immutable so only one instance is needed for
     * all uses.
     */
        ...
        try {
            ...
            while (ldifReader.hasNext()) {
                if (first) {
                    ...
                    // throw an exception : we should not have more than one
                    // entry per schema ldif file
    ...
    /**
     * Gets the unique schema file resource from the class loader off the base
     * path. If
     * the same resource exists multiple times then an error will result since
     * the resource
     * is not unique.
     *
     * @param resourceName
     ...
     * @throws IOException
     *             if there are problems reading or finding a unique copy of the
     *             resource
     */
    ...
    public static URL getUniqueResource(final String resourceName, final String resourceDescription) throws IOException {
        ...
        if (resources.hasMoreElements()) {
            ...
            // throw new UniqueResourceException( resourceName, result,
